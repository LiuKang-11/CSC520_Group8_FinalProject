...
def run_scenario(scenario_name, save_metrics=True):
    scenarios = {
        '1': simple_crossing,
    ...
    if scenario_name in scenarios:
        ...
        if save_metrics and metrics:
            save_metrics_to_file(metrics, scenario_name)
...
def simple_crossing():
    print("\n" + "="*60)
    print("SCENARIO 1: Simple Crossing")
    print("="*60)
    ...
    adapter = MinigridAdapter(env, algorithm='cbs')
    ...
    if solution:
        metrics = collect_metrics(adapter, env, planning_time, scenario='simple')
        print_solution_summary(adapter, env)
        visualize(env, solution, 'simple')
        return metrics
    else:
...
def collision_avoidance():
    print("\n" + "="*60)
    print("SCENARIO 2: Collision Avoidance (Position Swap)")
    print("="*60)
    ...
    if solution:
        print("\nCBS found collision-free solution:")
        print_paths(solution)
        metrics = collect_metrics(adapter, env, planning_time, scenario='collision')
        print_solution_summary(adapter, env)
        visualize(env, solution, 'collision', fps=1)
        return metrics
    else:
        print("No solution found")
        return None
...
def complex_obstacles():
    print("\n" + "="*60)
    print("SCENARIO 3: Complex with Obstacles")
    print("="*60)
    ...
    if solution:
        metrics = collect_metrics(adapter, env, planning_time, scenario='complex')
        print_solution_summary(adapter, env)
        visualize(env, solution, 'complex', fps=3)
        return metrics
    else:
        print("No solution found")
        return None
...
def print_solution_summary(adapter, env):
    print("\nSolution Summary:")
    total_planned = 0
    total_optimal = 0
...
def visualize(env, solution, name, fps=2):
    ...
    visualizer = MinigridVisualizer(env, solution)
    ...
    visualizer.save_trajectory_plot(f'{output_dir}/{name}_trajectories.png')
...
def collect_metrics(adapter, env, planning_time, scenario='unknown'):
...
def save_metrics_to_file(metrics, scenario_name):
    output_dir = 'minigrid_integration/output'
    os.makedirs(output_dir, exist_ok=True)

    filename = f'{output_dir}/metrics_{scenario_name}.json'

    with open(filename, 'w') as f:
        json.dump(metrics, f, indent=2)

    print(f"\n  Metrics saved: output/metrics_{scenario_name}.json")
